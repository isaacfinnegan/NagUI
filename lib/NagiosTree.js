...
Ext.define('NagUI.NagiosTree',
{
    ...
    constructor: function(config)
    {
        ...
        {
            Ext.apply(this, this.preloadedState);
        }
...
function nagiosStatusRender(value, o, node, row, col, store)
{
    ...
    if (r.comments_with_info && r.comments_with_info.length > 0)
    {
        ...
        {
            rtn += r.comments_with_info[i][2].replace(/"/g, "'") + ' - <span class=signature><i>' + r.comments_with_info[i][1] + '</i></span><br/>';
        }
        rtn += '">';
    }

    // add dressing for disabled notifications
        ...
        (typeof r.host_scheduled_downtime_depth != 'undefined' && r.host_scheduled_downtime_depth > 0))
    {
        ...
        {
            rtn += r.downtimes_with_info[i][2].replace(/"/g, "'") + ' - <span class=signature><i>' + r.downtimes_with_info[i][1] + '</i></span><br/>';
        }
        rtn += '">';
    }

    if (r.num_services_crit !== '' && r.num_services_ok !== '' && r.num_services_unknown !== '' && r.num_services_warn !== '')
    {
        if (r.num_hosts != '')
        {
    ...
    // do state counts for nodes that have thenm
    if (typeof r['state = 0'] != 'undefined' && typeof r['state = 1'] != 'undefined' && typeof r['state = 2'] != 'undefined' && typeof r['state = 3'] != 'undefined')
    {
