...
//converts a downtime to a serice or host node
function downtimeToNode(node)
{
    var newnode = {};
    if (node.data.is_service)
    {
        newnode = {
            host_name: node.data.host_name,
            description: node.data.service_description,
            nagios_type: 'service',
            state: node.data.service_state
        };
    }
    else
    {
...
// link to remove comment renderer
function removeDowntimeRender(value, o, node, row, col, store)
{
    var rtn = '<a href="javascript:void(0);" onClick="removeDowntime(\'' + (node.data.is_service ? "service" : "host") + '\',' + node.data.id + '); Ext.getCmp(\'' + this.getId() + '\').refresh();">remove</a>';
    return rtn;
}
