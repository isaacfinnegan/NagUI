								...
								items:[
									{
										...
										handler: function(){
											if (Ext.getCmp('nagios_search').store.getRootNode().childNodes[0].data.nagios_type == "host") {
												var data=[["Host Name","OK","Warn","Crit","Unknown","Notes","Nagios Peer"]];
												Ext.each(Ext.getCmp('nagios_search').store.getRootNode().childNodes,function(i){
													var a=i.copy();
													data.push([a.data.name,a.data.num_services_ok,a.data.num_services_warn,a.data.num_services_crit,a.data.num_services_unknown,"\""+a.data.notes+"\"",a.data.peer_name]);
													return;
												});
											} else if (Ext.getCmp('nagios_search').store.getRootNode().childNodes[0].data.nagios_type == "service") {
												var data=[["Host Name","Service Name","Output","Comments","Nagios Peer"]];
												Ext.each(Ext.getCmp('nagios_search').store.getRootNode().childNodes,function(i){
													var a=i.copy();
													data.push([a.data.host_name,a.data.description,"\""+a.data.plugin_output+"\"","\""+a.data.comments_with_info.join(";")+"\"",a.data.peer_name]);
													return;
												});
											} else {
												var data=[["Hostgroup Name","Hosts","Services OK","Services Warn","Services Crit","Services Unknown","Notes","Nagios Peer"]];
												Ext.each(Ext.getCmp('nagios_search').store.getRootNode().childNodes,function(i){
													var a=i.copy();
													data.push([a.data.name,a.data.num_hosts,a.data.num_services_ok,a.data.num_services_warn,a.data.num_services_crit,a.data.num_services_unknown,"\"a.data.notes\"",a.data.peer_name]);
													return;
												});
											}
											console.log(data);
											var csvContent = "data:text/csv;charset=utf-8,";
											data.forEach(function(dumpInfoArray, dumpIndex){
												dataString = dumpInfoArray.join(",");
												//csvContent += dumpIndex < dumpInfoArray.length ? dataString+ "\n" : dataString;
											...
											link.setAttribute("nagui_export", "my_data.csv");
											link.click(); */
										}
									},
									{
										...
										handler: function(){
											var nodes=Ext.getCmp('nagios_views').getActiveTab().getChecked();
											if (nodes[0].data.nagios_type == "host") {
												var data=[["Host Name","OK","Warn","Crit","Unknown","Notes","Nagios Peer"]];
												Ext.each(nodes,function(i){
													var a=i.copy();
													data.push([a.data.name,a.data.num_services_ok,a.data.num_services_warn,a.data.num_services_crit,a.data.num_services_unknown,"\"a.data.notes\"",a.data.peer_name]);
											...
											} else if (nodes[0].data.nagios_type == "service") {
												var data=[["Host Name","Service Name","Output","Comments","Nagios Peer"]];
												Ext.each(nodes,function(i){
													var a=i.copy();
													data.push([a.data.host_name,a.data.description,"\""+a.data.plugin_output+"\"","\""+a.data.comments_with_info.join(";")+"\"",a.data.peer_name]);
                                ...
                                var w = Ext.create('Ext.window.Window',
                                {
                                    ...
                                    tbar: [
                                    {
                                        ...
                                        menu:
                                        {
                                            items: [
                                            {
                                                text: 'View Online Docs for LQL node query language',
                                                handler: function()
                                                {
                                                    window.open('http://mathias-kettner.de/checkmk_livestatus.html#H1:LQL%20-%20The%20Livestatus%20Query%20Language', '_blank', '');
                                                }
                                            }]
                                        }
                                    }],
                                    items: [
                                    {
                                        ...
                                        {
                                            text: 'http://mathias-kettner.de/checkmk_livestatus.html#H1:LQL%20-%20The%20Livestatus%20Query%20Language'
                                        }],
                                        buttons: [
                                        {
                                            text: 'Add',
                                            handler: function()
                                            {
                                                ...
                                                var customnode = tab.getRootNode().appendChild(
                                                {
                                                    ...
                                                    parms:
                                                    {
                                                        ...
                                                        status: (formvals.query.match(/^get hosts/i) ? '' : 'plugin_output')
                                                    }
                                                });
                                                customnode.expand();
            ...
            },
            {
                ...
                menu:
                {
                    ...
                    items: [
                    {
                        ...
                        handler: function(i)
                        {
                            this.parentMenu.itemHandler(i);
                        }
                    },
                    {
                        id: 'servicegroup',
                        text: 'Search servicegroups:',
                        iconCls: 'servicegroup',
                        handler: function(i)
                        {
                            this.parentMenu.itemHandler(i);
                        }
