        ...
        items: [
            ...
            new Ext.FormPanel(
            {
                ...
                },
                {
                    fieldLabel: 'Sticky',
                    name: 'sticky',
                    checked: NagUI.config.acknowledge_defaults.sticky
                },
                {
                    fieldLabel: 'Notify',
                    name: 'notify',
                    checked: NagUI.config.acknowledge_defaults.notify
                },
                {
                    fieldLabel: 'Persist',
                    name: 'persist',
                    checked: NagUI.config.acknowledge_defaults.persist
                },
                {
                    xtype: 'fieldset',
                    title: 'Ticket',
                ...
                buttons: [
                {
                    ...
                    handler: function(b, e)
                    {
                        ...
                        if (typeof ackTicket == 'function' && formData.ticket == 'on')
                        {
                            ...
                            NagUI.log(formData);
                            ackTicket(b.ackNode, formData, function(k, d, resultMessage)
                            {
                                formData.ack_text += ' ' + k;
                                ...
                                // 	resultMessage: k + '<br /><a target="_blank" href="' + link + '">' + link + '</a>' 
                                // });		
                                Ext.MessageBox.show(resultMessage);
                            });
                        }
                        else
                        {
...
function scheduleRecheck(node)
{
    ...
    Ext.each(nodelist, function(n)
    {
        ...
        if (n.data.nagios_type == 'host')
        {
            str += 'SCHEDULE_HOST_SVC_CHECKS;' + n.data.name + ';' + (dt * 1 + 10);
        }
        if (n.data.nagios_type == 'service')
        {
            str += 'SCHEDULE_SVC_CHECK;' + n.data.host_name + ';' + n.data.description + ';' + (dt * 1 + 20);
        }
    });
    sendNagiosCommand(str);
}

// removing downtime is a recursive operation, as setting downtime for a host + its services can be
// done in one command, there's no similar command to remove it. so this function expands hosts
// to fetch the services then steps through them to assemble to command strings to remove downtime for 
// host + its services.  All this is skipped if a downtime id is passed.  Node is still needed to 
// determine correct command (host vs svc)
// if an id is passed in, the node must be a string specifying the nagios_type:  host | service
function removeDowntime(node, id)
{
    ...
    // remove specific downtime when id is passed in
    if (id && typeof node == 'string')
    {
        str += 'COMMAND [' + dt + '] ';
        if (node == 'host')
        {
            str += 'DEL_HOST_DOWNTIME;' + id;
        }
        if (node == 'service')
        {
            str += 'DEL_SVC_DOWNTIME;' + id;
        }
    }
    else
    {
        var nodelist;
        // check for array of nodes sent in
        if (node.length > 0)
        {
            nodelist = node;
        }
        ...
        if (NagUI.processingBatch)
        {

        }
        else
        {
            NagUI.processingBatch = true;
            Ext.MessageBox.show(
            {
                msg: 'Removing Downtimes',
        ...
        {
            NagUI.log(n);
            // check to see if node is host, if so
            // expand and remove downtime from all servies
            if (n.get('nagios_type') == 'host')
            {
                if (n.isExpanded())
                {
                    removeDowntime(n.childNodes);
                }
                else
                {
                    n.expand(
            ...
            // loop through available downtimes on node and assemble removals
        ...
        if (NagUI.processingBatch)
        {
            NagUI.processingBatch = {
                run: function()
                {
                    if (!Ext.Ajax.isLoading())
                    {
                        Ext.resumeLayouts();
                        Ext.TaskManager.stop(NagUI.processingBatch);
                        NagUI.processingBatch = false;
                        Ext.MessageBox.hide();
                    }
                },
                interval: 1000
            };
            Ext.TaskManager.start(NagUI.processingBatch);
        }
    }
    sendNagiosCommand(str);
}

// optional id parameter will allow for directly deleting specific comment. If passsed,
// node must =  string: 'host' | 'service'
function removeComments(node, id)
{
    var str = '';
    var dt = new Date();
    dt = dt.format('U');
    if (id && typeof node == 'string')
    {
        ...
        if (node == 'host')
        {
            str += 'DEL_HOST_COMMENT;' + id;
        }
        if (node == 'service')
        {
            str += 'DEL_SVC_COMMENT;' + id;
        }
    }
    else
    {
...
function toggleAlerts(node, enabledisable)
{
    ...
    Ext.each(nodelist, function(n)
    {
        ...
        if (n.data.nagios_type == 'servicegroup')
        {
            str += enabledisable + '_SERVICEGROUP_SVC_NOTIFICATIONS;' + n.data.name;
        }
    });
    sendNagiosCommand(str);
}
...
function scheduleNodeString(node, opt)
{
    ...
    Ext.each(nodelist, function(n)
    {
        ...
        if (n.data.nagios_type == 'servicegroup')
        {
            str += 'SCHEDULE_SERVICEGROUP_SVC_DOWNTIME;' + n.data.name + ';' + start + ';' + end + ';1;0;0;USERNAME;' + opt.comment;
        }
    });
    return str;
}
