...
//converts a comment into a service or host node
function commentToNode(node)
{
    var newnode = {};
...
function removeCommentRender(value, o, node, row, col, store)
{
    var rtn = '<a href="javascript:void(0);" onClick="removeComments(\'' + (node.data.is_service ? "service" : "host") + '\',' + node.data.id + '); Ext.getCmp(\'' + this.getId() + '\').refresh();">delete</a>';
    return rtn;
}
