...
NagUI.nodeQueries.servicegroups = 'GET servicegroups|Columns: name num_services_ok num_services_warn num_services_crit num_services_unknown worst_service_state';
...
Ext.define('nagios_node',
{
    ...
    fields: [
        'acknowledged', 'address', 'author', 'check_command', 'comments_with_info', 'description', 'downtimes_with_info',
        'duration', 'groups', 'host_address', 'host_groups', 'host_name', 'host_notes', 'host_scheduled_downtime_depth', 'worst_service_state',
        {
            name: 'last_check',
            type: 'date',
        ...
        // nagios_type determined based on fields returned. needs to be last since fields are processed in order
        {
            ...
            convert: function(v, node)
            {
                ...
                if (typeof node.data.worst_service_state == "number")
                {
                    nt = 'servicegroup';
                }
                if (node.data.meta > 0)
        ...
        },
        {
            ...
            convert: function(v, node)
            {
                ...
                return (
                    (node.get('nagios_type') == 'service' && node.data.num_services_ok == "") || node.get('nagios_type') == 'comment' || node.get('nagios_type') == 'downtime'
                ) ? true : false;
            }
        },
        {
...
Ext.define('NagUI.NagiosStore',
{
    ...
    makeNodeQuery: function(node)
    {
        ...
        if (node.data.parms)
        {
            ...
            if (parms.query)
            {
                parms.query = NagUI.config.nodequeries[parms.query] || NagUI.nodeQueries[parms.query] || parms.query;
                ...
                if (parms.query.match('GET comments'))
                {
                    parms.return_type = 'comments';
                    parms.groupby = '';
                    parms.nodetext = 'author';
                    parms.status = 'comment';
                }
                if (parms.query.match('GET downtimes'))
                {
                    parms.return_type = 'downtime';
                    parms.groupby = '';
                    parms.nodetext = 'author';
                    parms.status = 'comment';
                }
        ...
        if (node.data.query)
        {
            parms.query =  NagUI.config.nodequeries[node.data.query] || NagUI.nodeQueries[node.data.query] || node.data.query;
        ...
        if (typeof node.data.peer_name != 'undefined' && node.data.peer_name.split(' ').length == 1 && node.data.peer_name.length > 3 && node.data.nagios_type != 'meta' && node.data.nagios_type != 'servicegroup' && node.data.nagios_type != 'hostgroup')
        {
