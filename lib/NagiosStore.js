...
NagUI.nodeQueries.comments='GET comments|Columns: author host_name host_notes host_address comment';
NagUI.nodeQueries.downtimes='GET downtimes|Columns: author host_name host_notes host_address comment duration';

Ext.define('nagios_node',{
	extend: 'Ext.data.Model',
	fields: [
		'acknowledged',	'address','author','check_command','comments_with_info','description','downtimes_with_info',
		'duration','groups','host_address','host_groups','host_name','host_notes','host_scheduled_downtime_depth',
		...
		}},
		{
			name: 'leaf', convert: function(v,node){
				return (
					( node.get('nagios_type') == 'service' && node.data.num_services_ok == "") 
					|| node.get('nagios_type') == 'comment' || node.get('nagios_type') == 'downtime'
				) ? true : false;
			}
		}
		,
