...
Ext.define('NagUI.SavedViews', {
    ...
    listeners: {
        itemclick: {
            fn: function(view, rec, item) {
                if (rec.data.leaf) {
                    var viewLkup = Ext.getCmp('nagios_views').items.findIndex('stateId', rec.data.text, 0, false, true);
                    if (viewLkup >= 0) {
                        Ext.getCmp('nagios_views').setActiveTab(viewLkup);
                    } else {
                        var newview = addCustomView(rec.data.text, {
                            preloadedState: rec.data.viewstate
                        });
                        newview.applyState(rec.data.viewstate, true);
                    }
                }
            }
        },
    ...
    persistSavedViews: function(view) {
        var savedViews = [];
        var parentNode;
        if (view != 'shared') {
            view = NagUI.username;
