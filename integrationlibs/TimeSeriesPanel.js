...
Graphite.timeseriesWindow = function(n, name_key, title)
{
    var nodelist;
    ...
    var g = Ext.create('Graphite.TimeSeriesPanel',
    {
        title: undefined
    });
...
Ext.define('Graphite.TimeSeriesPanel',
{
    extend: 'Ext.panel.Panel',
    alias: 'widget.timeseriespanel',
    frame: true,
    layout: 'border',
    currentGraph:
    {
    ...
    graphConfig:
    {
        graphite_url: Graphite.config.path,
        from: '-72hours',
        until: 'now',
        bgcolor: '#FFF',
        fgcolor: '#000',
        ...
        targets: [
        {
            name: 'cpu-system',
            color: '#CC6699',
            target: 'cactiStyle(aliasByNode(collectd.admin016*.cpu-avg.cpu-system.value,3))',
        },
        {
            name: 'cpu-user',
            color: '#2FFF00',
            target: 'cactiStyle(aliasByNode(collectd.admin016*.cpu-avg.cpu-user.value,3))'
        },
        {
            name: 'cpu-wait',
            color: '#0000FF',
            target: 'cactiStyle(aliasByNode(collectd.admin016*.cpu-avg.cpu-wait.value,3))'
        }],
    ...
    listeners:
    {
        'resize':
        {
            fn: function(p, w, h)
            {
                if (p.hschart)
                {
                    p.hschart.setSize(p.centerRegion.getBox().width, p.centerRegion.getBox().height);
                }
                else
                {
                    p.setGraph();
            ...
            menu:
            {
                ...
                listeners:
                {
                    'hide':
                    {
                        fn: function(menu)
                        {
                            menu.up('timeseriespanel').optionsPanel.collapse();
                        }
                    }
                },
                items: [
                {
                    ...
                    handler: function(i)
                    {
                        var gpanel = i.up('panel[xtype=timeseriespanel]');
        ...
        this.metricsBrowserSpec = {
            ...
            listeners:
            {
                'itemclick':
                {
                    ...
                    {
                        console.log(rec);
                        var gpanel = view.up('panel[xtype=timeseriespanel]');
    ...
    setGraph: function()
    {
        ...
        this.graphConfig.targets = [];
        for (var l = 0; l < targets.length; l++)
        {
            var fqdn = targets[l].split('.');
            if (this.templateGraph)
            {
                this.graphConfig.title = this.templateGraph.get('title');
                this.graphConfig.vtitle = this.templateGraph.get('vtitle');
                var template_targets = this.templateGraph.get('target');
                for (var tl = 0; tl < template_targets.length; tl++)
                {
                    this.graphConfig.targets.push(
                    {
                        // name: 
                        // color:
                        target: template_targets[tl].replace(/{{id}}/g, fqdn[0] + '*')
                    });
                }
            }
            else
            {
                this.graphConfig.targets = [
                {
                    target: Graphite.config.metricsPrefix + "." + fqdn[0] + "*." + this.currentGraph.metric
                }];
                // graphUrl += "&title=" + this.currentGraph.metric;
            }

        }
        // yes this calls out to jquery stuff 
        $("#" + this.centerRegion.body.id).graphiteHighcharts(this.graphConfig, function(err)
        {
            console.log(err);
        }, this);
